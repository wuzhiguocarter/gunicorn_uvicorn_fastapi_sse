...
async def test_client_metrics():
    ...
    async with ChatBotLoadTester("http://localhost:8000") as tester:
        ...
        print("\n📊 测试结果:")
        print(
            f"   {'编号':<5} {'成功':<6} {'事件数':<8} {'响应时间':<12} {'完整事件':<8}"
        )
        print(f"   {'-' * 45}")
        ...
        for i, result in enumerate(results):
            ...
            print(
                f"   {i + 1:<5} {success:<6} {event_count:<8} {response_time:<12} {complete:<8}"
            )
        ...
        print(
            f"   验证成功率: {(metrics.successful_requests / metrics.total_requests * 100):.1f}%"
        )
        ...
        if calculated_failed == metrics.failed_requests:
            ...
            print(
                f"   ❌ 数据不一致: calculated_failed={calculated_failed}, metrics.failed_requests={metrics.failed_requests}"
            )
