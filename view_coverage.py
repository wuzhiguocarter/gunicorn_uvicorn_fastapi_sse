...
def view_coverage():
    ...
    try:
        ...
        # 计算总体统计
        total_statements = sum(metrics['index']['nums']['n_statements'] for metrics in status['files'].values())
        total_missing = sum(metrics['index']['nums']['n_missing'] for metrics in status['files'].values())
