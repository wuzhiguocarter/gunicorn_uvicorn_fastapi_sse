...
class RampUpLoadTester:
    ...
    async def run_single_test_phase(self,
                                  ...
                                  phase_name: str = "",
                                  phase_num: int = 1,
                                  total_phases: int = 1) -> Dict[str, Any]:
        """运行单个测试阶段"""
        print(f"\n{'='*80}")
        ...
        # 显示进度条
        print(f"\n⏳ 正在执行测试...")
        progress_bar = tqdm(total=duration, desc=f"🔄 {phase_name}",
                           unit="s", bar_format="{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}]")
        ...
        async with ChatBotLoadTester(self.base_url) as tester:
            ...
            print(f"   {'⚡ 平均响应时间':<15} {metrics['average_response_time']:>6.3f}s")
            print(f"   {'📈 吞吐量':<15} {metrics['throughput']:>6.2f} req/s")
            print(f"   {'💻 CPU使用率':<15} {start_system_stats['cpu_percent']:>5.1f}% → {end_system_stats['cpu_percent']:>5.1f}%")
            print(f"   {'🧠 CPU空闲率':<15} {start_system_stats['cpu_idle']:>5.1f}% → {end_system_stats['cpu_idle']:>5.1f}%")
            print(f"   {'🗄️ 内存使用率':<15} {start_system_stats['memory_percent']:>5.1f}% → {end_system_stats['memory_percent']:>5.1f}%")
    ...
    async def run_ramp_up_test(self):
        ...
        try:
            ...
            overall_progress = tqdm(total=total_phases, desc="📊 整体进度",
                                  unit="阶段", bar_format="{l_bar}{bar}| {n_fmt}/{total_fmt}")
    ...
    def _build_report_content(self) -> str:
        ...
        if performance_knees:
            ...
            for knee in performance_knees:
                report_lines.append(f"### 第 {knee['phase']} 阶段 ({knee['concurrency']} 并发用户)")
        ...
        report_lines.append(f"- **最高吞吐量**: {best_throughput['concurrency']} 并发用户, {best_throughput['throughput']:.2f} req/s")
        report_lines.append(f"- **最高成功率**: {best_success_rate['concurrency']} 并发用户, {best_success_rate['success_rate']:.1f}%")
        report_lines.append("")

        # 性能趋势分析
        report_lines.append("## 📈 性能趋势分析")
        report_lines.append("")
        report_lines.append("| 并发用户 | 成功率 | 吞吐量 (req/s) | 响应时间 (s) | 内存使用率 |")
        report_lines.append("|----------|--------|----------------|---------------|------------|")
        for result in self.results:
            report_lines.append(f"| {result['concurrency']} | {result['success_rate']:.1f}% | {result['throughput']:.2f} | {result['avg_response_time']:.3f} | {result['end_memory_percent']:.1f}% |")
        report_lines.append("")
        ...
        if stable_phases:
            ...
            report_lines.append(f"✅ 系统在 {max_stable_concurrency['concurrency']} 并发用户内表现稳定")
        else:
        ...
        for i, result in enumerate(self.results):
            ...
            report_lines.append(f"- **最小响应时间**: {result['min_response_time']:.3f}s")
            report_lines.append(f"- **最大响应时间**: {result['max_response_time']:.3f}s")
            report_lines.append(f"- **吞吐量**: {result['throughput']:.2f} req/s")
            report_lines.append(f"- **CPU使用率**: {result['start_cpu_percent']:.1f}% → {result['end_cpu_percent']:.1f}%")
            report_lines.append(f"- **CPU空闲率**: {result['start_cpu_idle']:.1f}% → {result['end_cpu_idle']:.1f}%")
            report_lines.append(f"- **内存使用率**: {result['start_memory_percent']:.1f}% → {result['end_memory_percent']:.1f}%")
            ...
            if line.startswith('#') or line.startswith('|') or line.startswith('-'):
                ...
                if line.startswith('|'):
                    ...
                    if len(parts) >= 5:
                        ...
                        print(f"   {concurrency:>8} | {success_rate:>7} | {throughput:>7} | {response_time:>11} | {memory:>10}")
                else:
                    skip_next = False
                ...
                print(f"\n🏆 最佳性能点:")
            elif line.startswith('- **最高吞吐量**:') and '最佳性能点' in report_content:
                print(f"   • {line[3:]}")
            elif line.startswith('- **最高成功率**:') and '最佳性能点' in report_content:
                print(f"   • {line[3:]}")
...
async def main(args):
    ...
    # 检查服务器状态
    print("🔍 检查服务器状态...")
    health_check_progress = tqdm(total=10, desc="🔍 服务器健康检查",
...
示例用法:
  ...
  python ramp_up_test.py --no-prompt --url http://localhost:8080  # 非交互模式
    ...
    parser.add_argument(
        "--url",
        type=str,
        help="指定服务器URL（默认：http://localhost:8000）"
    )
